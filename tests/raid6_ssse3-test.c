...
int main()
{
	...
	for(i=0;i<N;i++) if( 0 != posix_memalign( (void **) &mem[i] , 16, PAGE_SIZE) ) { printf("mem fail\n"); exit(-1); }
	for(i=0;i<N+2;i++) if( 0 != posix_memalign( (void **) &mem2[i] , 16, PAGE_SIZE) ) { printf("mem fail\n"); exit(-1); }

	randomize_mem( mem , N-2);
	raid6_ssse3x2_gen_syndrome(N,PAGE_SIZE,(void **)mem );
...
///////////////////////////////////
	...
	/// checked code
	raid6_ssse3x2_1fail_recov(N,PAGE_SIZE,fail_idx[1],(void **)mem2 );
	...
	/// checked code
	raid6_ssse3x2_1fail_recov(N,PAGE_SIZE,IDXP,(void **)mem2 );
	...
	/// checked code
	raid6_ssse3x2_1fail_recov(N,PAGE_SIZE,IDXQ,(void **)mem2 );
...
//	raid6_ssse3x2_datap_recov(N,PAGE_SIZE,fail_idx[0],(void **)mem2 );
	raid6_ssse3x2_2fails_recov(N,PAGE_SIZE,fail_idx[0],IDXP,(void **)mem2 );
...
//	raid6_ssse3x2_2data_recov(N,PAGE_SIZE,fail_idx[0],fail_idx[1],(void **)mem2 );
	raid6_ssse3x2_2fails_recov(N,PAGE_SIZE,IDXQ,fail_idx[0],(void **)mem2 );
	assert(0==is_diff((uint64_t*)&mem[fail_idx[0]][64],(uint64_t*)&mem2[fail_idx[0]][64]));
	assert(0==is_diff((uint64_t*)&mem[IDXQ][64],(uint64_t*)&mem2[IDXQ][64]));
	...
	/// checked code
//	raid6_ssse3x2_2data_recov(N,PAGE_SIZE,fail_idx[0],fail_idx[1],(void **)mem2 );
	raid6_ssse3x2_2fails_recov(N,PAGE_SIZE,fail_idx[0],fail_idx[1],(void **)mem2 );
	assert(0==is_diff((uint64_t*)&mem[fail_idx[0]][64],(uint64_t*)&mem2[fail_idx[0]][64]));
	assert(0==is_diff((uint64_t*)&mem[fail_idx[1]][64],(uint64_t*)&mem2[fail_idx[1]][64]));
